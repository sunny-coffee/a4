...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        end_encoder_hidden = torch.cat([h_n[0], h_n[1]], dim=1)
        end_encoder_cell = torch.cat([c_n[0], c_n[1]], dim=1)
        init_decoder_hidden = self.h_projection(end_encoder_hidden)
        init_decoder_cell = self.c_projection(end_encoder_cell)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = self.model_embeddings.source(target_padded) # (tgt_len, b, e)
        for y in torch.split(Y, 1):    # (1,b,e)
            Y_t = torch.squeeze(y, dim=0)  # (b,e)
            # print(Y_t.shape, o_prev.shape)
            Ybar_t = torch.cat((Y_t, o_prev), dim=1) # (b,e+h) dim=1非常重要# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ###     Tensor Squeeze:
        ###         https://pytorch.org/docs/stable/torch.html#torch.squeeze
        # print(Ybar_t.shape, dec_state[0].shape, dec_state[1].shape)
        ...
        alpha_t = F.softmax(e_t, dim=1)                    # (b, seq)
        # alpha_t = torch.unsqueeze(alpha_t, dim=1)   # (b, 1, seq)
        a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1)      
        # a_t = torch.squeeze(a_t, dim=1)       # (b, 1, 2*h)       # (b, 2*h)
